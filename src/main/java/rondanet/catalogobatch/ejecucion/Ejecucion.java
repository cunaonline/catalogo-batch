	...
	public void runProgram(String[] parametrosEntradaCommandLine) {
		...
		try {
			...
			if (parametrosValidos) {
			    ...
			    for (String empresasProveedorasFichero : loadConfigIni.getEmpresas()) {	
				  //Obtener los productos por empresa
			    	
			    	List<ProductoBatchDTO> listaProductos = desacargarInformacion.descargarProductos(empresasProveedorasFichero,
							validarDatos.getParametros());

					// Generar Archivos
				  if (validarDatos.getParametros().get("formato").equals("CSV"))
					generarArchivo.crearArchivoCSV(listaProductos, loadConfigIni.getColumnas(), directorioCarpeta.getUrlbase(),empresasProveedorasFichero);
				  else if (validarDatos.getParametros().get("formato").equals( "DBF"))
					generarArchivo.crearArchivoDBF(listaProductos, loadConfigIni.getColumnas(), directorioCarpeta.getUrlbase(),empresasProveedorasFichero);	
				}
			}else {
